...
#I load in the VTR points, and create the convex hull using only points
#lying within the 95 percentile of the distance to the centroid. Then I 
#create them as shape files so as to be compatible with the biomass rasters. 
...
year_list = c(1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 
              2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 
              2020, 2021)
hull_poly_bounds<- list()
...
for(y in year_list){
  ...
  # write to shapefile
  st_write(hull_poly, dsn = paste0("//net.nefsc.noaa.gov/home2/aharris/DisMap data/VTR_chulls/hull_poly_",year,".shp"), 
           layer = paste0("hull_poly_",year), driver = "ESRI Shapefile", delete_layer = TRUE) 
  
  ##import and transform the hull poly's
  hull_poly_boundaries <- readOGR( dsn="\\\\net.nefsc.noaa.gov/aharris/DisMap data/VTR_chulls",
                                   layer=paste0("hull_poly_",year),
                                   stringsAsFactors=FALSE, verbose=FALSE)
  
  hull_poly_boundaries <- spTransform(hull_poly_boundaries, CRS=PROJ.USE)
  hull_poly_bounds[[y]] <- hull_poly_boundaries
}


VTR_boundaries = hull_poly_bounds[-which(sapply(hull_poly_bounds, is.null))]
names(VTR_boundaries) <- paste0('VTR_boundaries_',1996:2021)
...
file_list_SF_F = list.files(path = "\\\\net.nefsc.noaa.gov/aharris/DisMap data/Paralichthys dentatus_F", 
                            all.files = FALSE,
                            pattern = "*.tif$",
                            recursive = TRUE)

file_list_SF_S = list.files(path = "\\\\net.nefsc.noaa.gov/aharris/DisMap data/Paralichthys dentatus_S", 
                            all.files = FALSE,
                            pattern = "*.tif$",
                            recursive = TRUE)

file_list_BSB_F = list.files(path = "\\\\net.nefsc.noaa.gov/aharris/DisMap data/Centropristis striata_F", 
                             all.files = FALSE,
                             pattern = "*.tif$",
                             recursive = TRUE)

file_list_BSB_S = list.files(path = "\\\\net.nefsc.noaa.gov/aharris/DisMap data/Centropristis striata_S", 
                             all.files = FALSE,
                             pattern = "*.tif$",
                             recursive = TRUE)
...
for (x in file_list_SF_F) {
  ...
  assign(paste0("Raster_",name, sep=""), 
         raster(here("\\\\net.nefsc.noaa.gov/aharris/DisMap data/Paralichthys dentatus_F",x)))
  assign(paste0("Raster_",name, sep=""), 
...
for (x in file_list_BSB_F) {
  name = stri_sub(x, 1, nchar(x)-4)
  assign(paste0("Raster_",name, sep=""), 
         raster(here("\\\\net.nefsc.noaa.gov/aharris/DisMap data/Centropristis striata_F",x)))
  assign(paste0("Raster_",name, sep=""), 
         projectRaster(from=get(paste0("Raster_",name, sep="")),
                       crs=PROJ.USE))
}
...
for (x in file_list_SF_S) {
  name = stri_sub(x, 1, nchar(x)-4)
  assign(paste0("Raster_",name, sep=""), 
         raster(here("\\\\net.nefsc.noaa.gov/aharris/DisMap data/Paralichthys dentatus_S",x)))
  assign(paste0("Raster_",name, sep=""), 
         projectRaster(from=get(paste0("Raster_",name, sep="")),
                       crs=PROJ.USE))
}
...
for (x in file_list_BSB_S) {
  name = stri_sub(x, 1, nchar(x)-4)
  assign(paste0("Raster_",name, sep=""), 
         raster(here("\\\\net.nefsc.noaa.gov/aharris/DisMap data/Centropristis striata_S",x)))
  assign(paste0("Raster_",name, sep=""), 
         projectRaster(from=get(paste0("Raster_",name, sep="")),
                       crs=PROJ.USE))
}
...
###Years of trawl survey data for spring and fall for which we also have VTR data
year_list_F = c(1996, 1997, 1998, 1999,
                2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009,
                2010, 2011, 2012, 2013, 2014, 2015, 2016,       2018, 2019)

year_list_S = c(1996, 1997, 1998, 1999,
                2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009,
                2010, 2011, 2012, 2013,       2015, 2016, 2017, 2018, 2019)
...
for (yr in year_list_F){
  ...
  yr <-yr
  VTR_list <- yr-1996+1
  state_polys <- VTR_boundaries[[VTR_list]]
  
  
  for (s in state_list){
    s<-s
    state_poly <- state_polys[s, ]
...
write_xlsx(RESULTS_S_F,"\\\\net.nefsc.noaa.gov/aharris/DisMap data/RESULTS_S_F_VTRs.xlsx")
