...
GoM_shape <- st_read(dsn="U:/GOM-GB_Regulated_Mesh_Areas",
...
for (m in 1:dim(time)){
  ...
  ## make sure I have correctly bt_temps inside and outside the convex hull
  # 
...
lat_lons_all2<-lat_lons_all

lat_lons_all2$year <- format(tmp_df02_long$date, "%y") #extract the year 
lat_lons_all2$year <- format(as.Date(lat_lons_all2$date, format="%Y-%m-%d"),"%Y")

lat_lons_all3<-lat_lons_all2 %>% 
  dplyr::select(-geometry, -area)

lat_lons_all_59_69<-lat_lons_all3 %>% 
  dplyr::filter(year<=1969)

write_xlsx(lat_lons_all_59_69,"\\\\net.nefsc.noaa.gov/aharris/DisMap data/hist_daily_bt_tmps_raw_GoM_59_69.xlsx")


lat_lons_all_70_79<-lat_lons_all3 %>% 
  dplyr::filter(year>1969 & year<=1979)

write_xlsx(lat_lons_all_70_79,"\\\\net.nefsc.noaa.gov/aharris/DisMap data/hist_daily_bt_tmps_raw_GoM_70_79.xlsx")


lat_lons_all_80_89<-lat_lons_all3 %>% 
  dplyr::filter(year>1979 & year<=1989)

write_xlsx(lat_lons_all_80_89,"\\\\net.nefsc.noaa.gov/aharris/DisMap data/hist_daily_bt_tmps_raw_GoM_80_89.xlsx")


lat_lons_all_90_99<-lat_lons_all3 %>% 
  dplyr::filter(year>1989 & year<=1999)

write_xlsx(lat_lons_all_90_99,"\\\\net.nefsc.noaa.gov/aharris/DisMap data/hist_daily_bt_tmps_raw_GoM_90_99.xlsx")


lat_lons_all_00_09<-lat_lons_all3 %>% 
  dplyr::filter(year>1999 & year<=2009)

write_xlsx(lat_lons_all_00_09,"\\\\net.nefsc.noaa.gov/aharris/DisMap data/hist_daily_bt_tmps_raw_GoM_00_09.xlsx")


lat_lons_all_10_22<-lat_lons_all3 %>% 
  dplyr::filter(year>2009 )

write_xlsx(lat_lons_all_10_22,"\\\\net.nefsc.noaa.gov/aharris/DisMap data/hist_daily_bt_tmps_raw_GoM_10_22.xlsx")



#Now aggregate by day 
lat_lons_all_agg<-lat_lons_all3 %>% 
  dplyr::group_by(date) %>% 
  dplyr::summarise(mean_tmp=mean(bt_tmp), sd_tmp=sd(bt_tmp), .groups="drop")

write_xlsx(lat_lons_all_agg,"\\\\net.nefsc.noaa.gov/aharris/DisMap data/hist_daily_bt_tmps_agg_GoM.xlsx")
...
ggplot(east_coast) +
  ...
  geom_point(data = lat_lons_ready, aes(x = LON, y = LAT), color = "red", alpha = 0.3, show.legend = FALSE) +
  geom_sf(data = GoM_shape['AREANAME'], fill = "blue", alpha = 0.3, show.legend = FALSE) +
  geom_sf(data = mgt_boundaries_subset['AREANAME'], fill = "green", alpha = 0.3, show.legend = FALSE) +
