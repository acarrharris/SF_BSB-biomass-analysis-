...
vtr_lat_lons<-read_csv("//net.nefsc.noaa.gov/home2/aharris/DisMap data/Yearly_Cod_Recreational_Lat_Lon.csv")

lat_lons_ready <- vtr_lat_lons %>%
...
####Now find the overlap from the VTR convex hull and the projected bottom temps 
...
##Now find the intersection of these points with the convex hull
VTR_boundaries_subset<- st_as_sf(hull_poly3, coords = c('LON' ,'LAT' )) 
VTR_boundaries_subset <- VTR_boundaries_subset %>%
  st_as_sf(coords = c("LON", "LAT"), crs = 4326) %>%
  summarise(geometry = st_combine(geometry)) %>%
...
sp_points = st_as_sf(bt_temp_deltas,coords = c('lon' ,'lat' ))#make points spatial
st_crs(sp_points)= 4326
st_crs(VTR_boundaries_subset)= 4326

VTR_boundaries_subset=st_transform(VTR_boundaries_subset,st_crs(sp_points)) # Match the point and polygon CRS

bt_tmp2 <- sf::st_join(sp_points, VTR_boundaries_subset)
##now make sure I have correctly bt_temps inside and outside the convex hull
